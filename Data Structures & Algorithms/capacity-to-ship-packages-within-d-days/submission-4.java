//Question: If I put all packages on the ship in one day, the capacity is just the sum of all weights. Why not use that?
//Answer: The reason is that the question is asking for the minimum capacity, not just a capacity that works.
//The sum of all weights is the maximum possible capacity, but it is not the smallest one.
class Solution {

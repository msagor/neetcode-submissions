...
function remove(node):
  remove the node from LL by pointing its prev to its next
  before: left -- some_nodes -- node -- some_node --- right
  after: left -- some_nodes -- some_node --- right


function insert(node):
  insert the node before the right pointer
  before: left --- some_nodes --- right
  after: left -- some_nodes -- node -- right
...
function put(key, val):
  check if the map contains the key
    ...
    if key not contains:
      ...
      check capacity:
        if overflown:
          call remove (left.next)
          remove left.next from map

...
//Suman Bhunia
//comment added by chatGPT
//uses array as opposed to map
...
public int characterReplacement(String s, int k) {
    ...
    while (r < s.length()) {
        ...
        // Add the current right character to the frequency array
        cFreq[s.charAt(r) - 'A']++;
        ...
        while (l <= r) {
            ...
            if (length <= k + max_freq) {
                ...
                // No need to shrink the window anymore
                break;

            } else {

                // Current window needs more than k replacements,
                // so remove the leftmost character from the window
                cFreq[s.charAt(l) - 'A']--;
